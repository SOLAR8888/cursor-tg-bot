...
- **Чаты IDE не видны.** SDK видит только тех агентов, которые создавал сам. Чаты, начатые в IDE, отображаются только в IDE/Web. Workaround: создавать чаты через бот, продолжать в IDE через `Filter > Source > SDK`.
...
### v0.3 — Read-only история IDE

- [ ] Чтение SQLite-базы Cursor IDE (`%APPDATA%/Cursor/...`) для просмотра истории чатов, начатых в IDE.
- [ ] Только просмотр, без изменения. С предупреждением о нестабильности.
